        ...
        with pm.Model() as four_p_model:
            if prior_from_ki == None:
                ...
                one_t_s = BoundedNormal('one_t_s', mu = prior_from_ki['one_t_s'],
                                        sd = np.sqrt(prior_from_ki['one_t_s_std']))
                BoundedNormal = pm.Bound(pm.Normal, lower=0)
                s_d = BoundedNormal('s_d', mu = prior_from_ki['s_d'], sd =  
                                    np.sqrt(prior_from_ki['s_d_std']))
                k = BoundedNormal('k', mu = prior_from_ki['k'], sd =
                                  np.sqrt(prior_from_ki['k_std']))
                BoundedNormal = pm.Bound(pm.Normal, lower=1e+04)
                n_e = BoundedNormal('n_e', mu = prior_from_ki['n_e'],
                                    sd =np.sqrt(prior_from_ki['n_e_std']))
            ...
            trace = pm.sample(num_samples, target_accept=0.91,
                              tune=2000, return_inferencedata=False) #target_accept=0.99,
